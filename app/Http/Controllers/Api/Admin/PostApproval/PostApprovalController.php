...
class PostApprovalController extends Controller
{
    ...
    public function cancelApprovedPost(Request $request)
    {
        $user = getAuthenticatedUserId();
        $userId = $user->user_id;
            try {
                ...
                if ($donor_post) {
                    ...
                    $donor_post->isApproved = 0;
                    $donor_post->save();
                   
                        AuditTrail::create([
                            'user_id'    => $userId,
                            'action'     => 'Cancel approved post | post id: ' . $donor_post->donor_posts_id,
                            'status'     => 'success',
                            'ip_address' => $ipwhois['ip'],
                            'region'     => $ipwhois['region'],
                            'city'       => $ipwhois['city'],
                        ...
                        return response()->json([
                            'status'       => 'error',
                            'message'      => 'Post Approved Cancelled',
                        ], 200);
